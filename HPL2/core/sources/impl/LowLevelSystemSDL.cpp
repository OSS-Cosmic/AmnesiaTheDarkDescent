...
#endif
  assert(false);
  cPlatform::CreateMessageBox(eMsgBoxType_Error, _W("FATAL ERROR"), _W("%ls"),
...
void SetUpdateLogActive(bool abX) { gbUpdateLogIsActive = abX; }

bool GetUpdateLogActive() { return gbUpdateLogIsActive; }
...
void cScriptOutput::Clear() { msMessage = ""; }
...
iScript *cLowLevelSystemSDL::CreateScript(const tString &asName) {
  return hplNew(cSqScript,
                (asName, mpScriptEngine, mpScriptOutput, mlHandleCount++));
}

//-----------------------------------------------------------------------

bool cLowLevelSystemSDL::AddScriptFunc(const tString &asFuncDecl, void *pFunc) {
  if (mpScriptEngine->RegisterGlobalFunction(
          asFuncDecl.c_str(), asFUNCTION(pFunc), asCALL_STDCALL) < 0) {
