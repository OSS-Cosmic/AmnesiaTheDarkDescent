...
static VkCommandBuffer __AcquireCmd(struct RIDevice *device,
                                    struct RITransferCommandGroup *group) {
  ...
  if (!group->is_recording) {
    ...
    if (vkGetFenceStatus(device->vk.device, fence) == VK_NOT_READY) {
      VK_WrapResult(
          vkWaitForFences(device->vk.device, 1, &fence, VK_TRUE, UINT64_MAX));
    }

    group->staging_buffer_offset = 0;
    for (size_t j = 0;
         j < (size_t)arrlen(group->temporary_buffers[group->active_set]); j++) {
      vkDestroyBuffer(device->vk.device,
                      group->temporary_buffers[group->active_set][j].vk.buffer,
                      NULL);
      vmaFreeMemory(
          device->vk.vmaAllocator,
          group->temporary_buffers[group->active_set][j].vk.allocation);
    }
    arrsetlen(group->temporary_buffers[group->active_set], 0);

    VK_WrapResult(vkResetCommandPool(
        device->vk.device, group->cmd_pool[group->active_set].vk.pool, 0));
    VkCommandBufferBeginInfo beginInfo = {
        VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO};
    beginInfo.flags = VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT;
    VK_WrapResult(
        vkBeginCommandBuffer(group->cmd[group->active_set].vk.cmd, &beginInfo));
...
static void __AcquireCmdD3D12(struct RIDevice *device,
                              struct RITransferCommandGroup *group) {
...
static void __FreeTransferCommandGroup(struct RIDevice *device,
                                       struct RITransferCommandGroup *group);

static void __InitTransferCommandGroup(struct RIDevice *device,
                                       struct RITransferCommandGroup *group,
    ...
    for (size_t i = 0; i < RI_RESOURCE_MAX_SETS; i++) {
      {
        ...
        VK_WrapResult(vkCreateCommandPool(device->vk.device, &info, NULL,
                                          &group->cmd_pool[i].vk.pool));
      ...
      // Fence starts signalled so the first acquire doesn't block.
      {
        ...
        VK_WrapResult(vkCreateFence(device->vk.device, &info, NULL,
                                    &group->vk.fences[i]));
      }

      {
        VkSemaphoreCreateInfo info = {VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO};
        VK_WrapResult(vkCreateSemaphore(device->vk.device, &info, NULL,
                                        &group->vk.semaphores[i]));
      }
...
static void __FreeTransferCommandGroup(struct RIDevice *device,
  ...
  if (RIIsTargetSelected(RI_DEVICE_API_VK))
    for (size_t i = 0; i < RI_RESOURCE_MAX_SETS; i++) {
      for (size_t j = 0; j < (size_t)arrlen(group->temporary_buffers[i]); j++) {
        vkDestroyBuffer(device->vk.device,
                        group->temporary_buffers[i][j].vk.buffer, NULL);
        vmaFreeMemory(device->vk.vmaAllocator,
                      group->temporary_buffers[i][j].vk.allocation);
      }
      arrfree(group->temporary_buffers[i]);

      vkFreeCommandBuffers(device->vk.device, group->cmd_pool[i].vk.pool, 1,
                           &group->cmd[i].vk.cmd);
      vkDestroyCommandPool(device->vk.device, group->cmd_pool[i].vk.pool, NULL);

      vmaDestroyBuffer(device->vk.vmaAllocator,
                       group->staging_buffer[i].vk.buffer,
                       group->staging_buffer[i].vk.allocation);
  ...
  if (RIIsTargetSelected(RI_DEVICE_API_D3D12)) {
    for (size_t i = 0; i < RI_RESOURCE_MAX_SETS; i++) {
      ...
      arrfree(group->temporary_buffers[i]);
      if (!group->cmd[i].isEmpty())
        group->cmd[i].dispose(device);
      if (group->cmd_pool[i].d3d12.allocator)
        group->cmd_pool[i].dispose(device);
      if (!group->staging_buffer[i].isEmpty())
        group->staging_buffer[i].dispose(device);
    }
    if (group->d3d12_timeline.d3d12.fence)
      group->d3d12_timeline.dispose(device);
  }
#endif
}

static bool __AllocateFromStageBuffer(struct RIDevice *device,
                                      struct RITransferCommandGroup *group,
                                      size_t size, size_t alignment,
                                      struct RIMappedMemoryRange *out) {
  const size_t alignedSize = ALIGN_TO(size, alignment);
  const size_t alignedOffset =
      ALIGN_TO(group->staging_buffer_offset, alignment);
  ...
  out->data =
      (uint8_t *)group->staging_buffer[set].mappedAddress + alignedOffset;
...
static void __AllocateTemporaryBuffer(struct RIDevice *device,
                                      struct RITransferCommandGroup *group,
                                      size_t size,
                                      struct RIMappedMemoryRange *out) {
...
static void __ResolveStageMemory(struct RIDevice *device,
                                 struct RITransferCommandGroup *group,
                                 size_t size, size_t alignment,
                                 struct RIMappedMemoryRange *out) {
...
void RI_InitResourceUploader(struct RIDevice *device,
                             struct RIResourceUploader *res) {
  assert(res);
  ...
  __InitTransferCommandGroup(device, &res->upload_resource,
                             &device->queues[RI_QUEUE_GRAPHICS]);

  struct RIQueue *copyQueue = &device->queues[RI_QUEUE_COPY];
#if (DEVICE_IMPL_VULKAN)
  if (RIIsTargetSelected(RI_DEVICE_API_VK) &&
      copyQueue->vk.queue == VK_NULL_HANDLE)
  ...
  if (RIIsTargetSelected(RI_DEVICE_API_D3D12) &&
      !res->d3d12_mip_generation.init(*device))
    hpl::Warning("RI: failed to initialize D3D12 mip generation\n");
#endif
}

void RI_FreeResourceUploader(struct RIDevice *device,
                             struct RIResourceUploader *res) {
  assert(res);
...
void RI_ResourceBeginCopyBuffer(struct RIDevice *device,
                                struct RIResourceUploader *res,
                                struct RIResourceBufferTransaction *trans) {
  if (RIIsTargetSelected(RI_DEVICE_API_D3D12)) {
#if (DEVICE_IMPL_D3D12)
  ...
  __ResolveStageMemory(device, &res->upload_resource, trans->size, 4,
                       &trans->mapped);
}

void RI_ResourceEndCopyBuffer(struct RIDevice *device,
                              struct RIResourceUploader *res,
                              struct RIResourceBufferTransaction *trans) {
#if (DEVICE_IMPL_VULKAN)
  if (RIIsTargetSelected(RI_DEVICE_API_VK)) {
    ...
    struct RICmd *barrierCmd =
        &res->upload_resource.cmd[res->upload_resource.active_set];
    ...
    vkCmdCopyBuffer(cmd, trans->mapped.buffer.vk.buffer,
                    trans->target.vk.buffer, 1, &region);

    if (trans->postState != RI_RESOURCE_STATE_UNDEFINED &&
        trans->postState != RI_RESOURCE_STATE_COPY_DST) {
...
void RI_ResourceBeginCopyTexture(struct RIDevice *device,
                                 struct RIResourceUploader *res,
                                 struct RIResourceTextureTransaction *trans) {
#if (DEVICE_IMPL_D3D12)
  if (RIIsTargetSelected(RI_DEVICE_API_D3D12)) {
    const struct RIFormatProps *formatProps = GetRIFormatProps(trans->format);
    const uint64_t alignedRowPitch = RIFormatAlignRowPitch(
        trans->rowPitch,
        device->physicalAdapter.uploadBufferTextureRowAlignment,
        formatProps->stride);
    const uint64_t alignedSlicePitch =
        (uint64_t)trans->sliceNum * alignedRowPitch;
    ...
    __AcquireCmdD3D12(device, &res->upload_resource);
    __ResolveStageMemory(device, &res->upload_resource, totalSize, offsetAlign,
                         &trans->mapped);
    return;
  }
#endif
  ...
  const struct RIFormatProps *formatProps = GetRIFormatProps(trans->format);
  const uint64_t alignedRowPitch = RIFormatAlignRowPitch(
      trans->rowPitch, device->physicalAdapter.uploadBufferTextureRowAlignment,
      formatProps->stride);
  const uint64_t alignedSlicePitch =
      (uint64_t)trans->sliceNum * alignedRowPitch;
  const uint32_t depth = trans->depth ? trans->depth : 1;
...
void RI_ResourceEndCopyTexture(struct RIDevice *device,
                               struct RIResourceUploader *res,
                               struct RIResourceTextureTransaction *trans) {
#if (DEVICE_IMPL_D3D12)
  if (RIIsTargetSelected(RI_DEVICE_API_D3D12)) {
    ...
    barrierCmd->copyBufferToTexture(device, &trans->mapped.buffer,
                                    &trans->target, desc);
  ...
  vkCmdCopyBufferToImage(cmd, trans->mapped.buffer.vk.buffer,
                         trans->target.vk.image,
...
bool RI_FormatSupportsMipGeneration(struct RIDevice *device, uint32_t format) {
  ...
  vkGetPhysicalDeviceFormatProperties(device->physicalAdapter.vk.physicalDevice,
                                      RIFormatToVK(format), &properties);
...
void RI_ResourceGenerateMips(struct RIDevice *device,
                             struct RIResourceUploader *res,
                             struct RIGenerateMipsDesc *desc) {
#if (DEVICE_IMPL_D3D12)
  if (RIIsTargetSelected(RI_DEVICE_API_D3D12)) {
    assert(desc);
    if (!desc || desc->mipNum <= 1)
      return;
    if (!res->d3d12_mip_generation.isReady())
      return;
    __AcquireCmdD3D12(device, &res->upload_resource);
    if (!res->d3d12_mip_generation.generate(
            *device, res->upload_resource.cmd[res->upload_resource.active_set],
            uint32_t(res->upload_resource.active_set), *desc))
  ...
  for (uint32_t i = 1; i < desc->mipNum; i++) {
    ...
    region.srcOffsets[1] = {(int32_t)srcWidth, (int32_t)srcHeight,
                            (int32_t)srcDepth};
    region.dstSubresource.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
    region.dstSubresource.mipLevel = i;
    region.dstSubresource.baseArrayLayer = desc->arrayOffset;
    region.dstSubresource.layerCount = layerNum;
    region.dstOffsets[1] = {(int32_t)dstWidth, (int32_t)dstHeight,
                            (int32_t)dstDepth};

    vkCmdBlitImage(cmd, desc->target.vk.image,
                   VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, desc->target.vk.image,
                   VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region,
                   VK_FILTER_LINEAR);
...
RI_VKFlushResourceUpdate(struct RIDevice *device,
                         struct RIResourceUploader *res, size_t num_semaphores,
  ...
  assert(vkGetFenceStatus(device->vk.device, group->vk.fences[active_set]) ==
         VK_SUCCESS);
  VK_WrapResult(
      vkResetFences(device->vk.device, 1, &group->vk.fences[active_set]));
  VK_WrapResult(vkQueueSubmit2(group->queue->vk.queue, 1, &submitInfo,
                               group->vk.fences[active_set]));
...
#if (DEVICE_IMPL_D3D12)
struct RIResourceUploaderD3D12Result
RI_D3D12FlushResourceUpdate(struct RIDevice *device,
                            struct RIResourceUploader *res) {
  ...
  RITimelineOp signal = {&group->d3d12_timeline, group->d3d12_timeline.next(),
                         RI_STAGE_COPY};
  RISubmitDesc desc = {};
  desc.cmds = cmds;
  desc.cmdCount = 1;
