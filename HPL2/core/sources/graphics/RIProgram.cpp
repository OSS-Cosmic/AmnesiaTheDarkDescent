...
static void ri_d3d12_replace_slot(ID3D12Resource *&oldResource,
  ...
  if (allocation) allocation->AddRef();
  if (oldResource) oldResource->Release();
  if (oldAllocation) oldAllocation->Release();
...
static void ri_d3d12_release_cache_refs(
...
static D3D12_CPU_DESCRIPTOR_HANDLE ri_d3d12_cpu(ID3D12DescriptorHeap *heap,
...
static D3D12_GPU_DESCRIPTOR_HANDLE ri_d3d12_gpu(ID3D12DescriptorHeap *heap,
                                                uint32_t index, uint32_t stride) {
...
static bool ri_d3d12_resolveBufferShape(
  ...
  const bool reflectedStructured = type.find("structured") != std::string::npos ||
                                   reflection->stride != 0;
  if (reflectedRaw == reflectedStructured ||
      (reflectedStructured && reflection->stride == 0)) {
    Error("RIProgram: storage-buffer reflection has an ambiguous/unsupported shape\n");
    return false;
  }
  ...
  if ((payload.raw && !shape.raw) || (payload.structured && shape.raw) ||
    ...
    Error("RIProgram: storage-buffer payload shape conflicts with reflection\n");
    return false;
  }
  return true;
}
...
static bool ri_d3d12_texture_view(D3D12_SHADER_RESOURCE_VIEW_DESC &desc,
  ...
  const auto type = static_cast<RITextureViewType_e>(texture.viewType);
  desc = {};
  desc.Format = ri_d3d12_srv_format(static_cast<DXGI_FORMAT>(texture.nativeFormat));
...
static bool ri_d3d12_texture_uav(D3D12_UNORDERED_ACCESS_VIEW_DESC &desc,
  ...
  case RI_VIEWTYPE_SHADER_RESOURCE_STORAGE_3D:
  {
    ...
    if (texture.baseMip >= rd.MipLevels || texture.baseMip >= 32)
      return false;
    desc.Texture3D.WSize = std::max<UINT>(1, rd.DepthOrArraySize >> texture.baseMip);
    return true;
  }
  default:
    return false;
...
static RIShaderArtifactFormat ri_detectShaderFormat(std::span<const char> data) {
  if (data.size() >= 4) {
    uint32_t magic = 0;
...
static std::string_view ri_metadataValue(std::string_view text, const char *key) {
...
static RIShaderArtifactFormat ri_readShaderMetadata(cFileSearcher *searcher,
  ...
  if (metadataSize == 0 ||
      !cPlatform::CopyFileToBuffer(metadataPath, metadata.data(), metadataSize)) {
  ...
  const std::string version = "version=" +
      std::to_string(RI_SHADER_ARTIFACT_VERSION) + "\n";
  ...
  if (format == RIShaderArtifactFormat::Dxil) {
    ...
    if (!ri_parseShaderArtifactEntries(ri_metadataValue(text, "entry="),
      ...
      FatalError("RIProgram: DXIL metadata has an unusable stage/entry map for '%s'\n",
                 artifactName.c_str());
    ...
    if (reflectionEnd == reflectionPos + strlen("reflection=") ||
      ...
      FatalError("RIProgram: DXIL metadata has no reflection linkage for '%s'\n",
                 artifactName.c_str());
    ...
    if (!cPlatform::CopyFileToBuffer(reflectionPath, reflection.data(), reflectionSize))
...
static bool ri_isArrayCount(const rapidjson::Value *v, uint32_t &count) {
  ...
  if (v->IsUint()) { count = v->GetUint(); return false; }
...
static void ri_find_layout(const rapidjson::Value &v, uint32_t &stride,
                           uint32_t &size) {
  if (v.IsArray()) {
    for (const auto &item : v.GetArray()) ri_find_layout(item, stride, size);
    return;
  }
  ...
  for (auto it = v.MemberBegin(); it != v.MemberEnd(); ++it)
    if (it->value.IsObject() || it->value.IsArray()) ri_find_layout(it->value, stride, size);
}
...
static bool ri_find_resource_shape(const rapidjson::Value &v,
  ...
  if (v.IsArray()) {
    for (const auto &item : v.GetArray())
      if (ri_find_resource_shape(item, shape)) return true;
    return false;
  }
...
static const rapidjson::Value *ri_find_named_parameter(
    const rapidjson::Value *parameters, std::string_view name) {
  if (!parameters || !parameters->IsArray()) return nullptr;
...
static void ri_find_push_constant(const rapidjson::Value &v,
                                  RIProgram::ShaderPushConstantReflection &out) {
  if (v.IsArray()) {
    for (const auto &item : v.GetArray()) ri_find_push_constant(item, out);
    return;
  }
  if (!v.IsObject()) return;
  ...
  const bool productionPush = name && name->IsString() &&
      std::string_view(name->GetString()) == "gPushConstants" &&
      bindingKind && bindingKind->IsString() &&
  ...
  if (explicitPush || productionPush) {
    ...
    if (binding) {
      ...
      out.registerIndex = ri_uint(ri_member(*binding, "index"), out.registerIndex);
      out.registerIndex = ri_uint(ri_member(*binding, "register"), out.registerIndex);
      out.registerIndex = ri_uint(ri_member(*binding, "registerIndex"), out.registerIndex);
      out.registerSpace = ri_uint(ri_member(*binding, "space"), out.registerSpace);
    }
  ...
  for (auto it = v.MemberBegin(); it != v.MemberEnd(); ++it)
    if (it->value.IsObject() || it->value.IsArray()) {
      if (it->value.IsArray())
        for (const auto &item : it->value.GetArray()) ri_find_push_constant(item, out);
      else
        ri_find_push_constant(it->value, out);
    }
}

static void ri_find_vertex_inputs(const rapidjson::Value &v,
                                  std::vector<RIProgram::ShaderVertexInputReflection> &out) {
  if (v.IsArray()) {
    for (const auto &item : v.GetArray()) ri_find_vertex_inputs(item, out);
    return;
  }
  if (!v.IsObject()) return;
  ...
  if (semantic && semantic->IsString() && kind && kind->IsString() &&
      std::string_view(kind->GetString()) == "varyingInput" &&
      index && index->IsUint()) {
    ...
    const bool duplicate = std::any_of(out.begin(), out.end(), [&](const auto &item) {
      return item.location == input.location;
    });
    if (!duplicate) out.push_back(std::move(input));
  }
  for (auto it = v.MemberBegin(); it != v.MemberEnd(); ++it)
    if (it->value.IsObject() || it->value.IsArray())
...
ri_parseShaderReflection(const std::string &json, const char *requestedEntry,
  ...
  if (!entries || !entries->IsArray()) FatalError("RIProgram: reflection has no entryPoints\n");
  ...
  for (const auto &candidate : entries->GetArray()) {
    ...
    if (!name || !name->IsString() ||
      ...
      continue;
    if (entry) FatalError("RIProgram: requested entry point is ambiguous in reflection\n");
    entry = &candidate;
  }
  if (!entry) FatalError("RIProgram: requested entry point is absent from reflection\n");

  auto result = std::make_shared<RIProgram::ShaderReflection>();
  result->json = std::make_shared<const std::string>(json);
  result->entryPoint = ri_member(*entry, "name") && (*entry)["name"].IsString() ? (*entry)["name"].GetString() : "";
  result->stage = ri_member(*entry, "stage") && (*entry)["stage"].IsString() ? (*entry)["stage"].GetString() : "";
  ...
  if (bindings && bindings->IsArray()) for (const auto &binding : bindings->GetArray()) {
    ...
    if (kind && kind->IsString()) {
      ...
      if (k == "constantBuffer") resource.registerClass = RIProgram::ShaderRegisterClass::CBV;
      else if (k == "shaderResource") resource.registerClass = RIProgram::ShaderRegisterClass::SRV;
      else if (k == "unorderedAccess") resource.registerClass = RIProgram::ShaderRegisterClass::UAV;
      else if (k == "samplerState") resource.registerClass = RIProgram::ShaderRegisterClass::Sampler;
    }
    resource.registerIndex = ri_uint(b ? ri_member(*b, "index") : nullptr);
    resource.registerSpace = ri_uint(b ? (ri_member(*b, "space") ? ri_member(*b, "space") : ri_member(*b, "registerSpace")) : nullptr);
    ...
    if (!parameter)
      if (const auto *globalScope = ri_member(*document, "globalScope"))
        parameter = ri_find_named_parameter(ri_member(*globalScope, "parameters"),
                                             resource.name);
    if (parameter) {
...
void ri_vk_buildGraphicsPipeline(const RIGraphicsPipelineDesc &desc,
  ...
  out.vertexInput.vertexBindingDescriptionCount =
  ...
  out.rendering.depthAttachmentFormat =
      rt.depthFormat == RI_FORMAT_UNKNOWN ? VK_FORMAT_UNDEFINED
                                          : RIFormatToVK(rt.depthFormat);
  out.rendering.stencilAttachmentFormat =
      rt.stencilFormat == RI_FORMAT_UNKNOWN ? VK_FORMAT_UNDEFINED
...
void RIProgram::bindDescriptors(struct RIDevice* device, struct RICmd* cmd, uint32_t frameIndex, DescriptorBinding* bindings, size_t bindingCount, VkPipelineBindPoint bindPoint) {
  ...
  if (RIIsTargetSelected(RI_DEVICE_API_D3D12)) {
    ...
    auto reflectedResource = [this](const BindingReflection *binding) -> const ShaderResourceReflection * {
      if (!binding) return nullptr;
      for (const auto &binary : shaderBin) if (binary.reflection) {
        for (const auto &resource : binary.reflection->resources) {
          const DescriptorBindingID id = CreateDescriptorBindingID(resource.name.c_str());
          if (id.hash == binding->hash &&
              static_cast<uint32_t>(resource.registerClass == ShaderRegisterClass::CBV ? RIBindlessRegisterClass::CBV :
                                    resource.registerClass == ShaderRegisterClass::SRV ? RIBindlessRegisterClass::SRV :
                                    resource.registerClass == ShaderRegisterClass::UAV ? RIBindlessRegisterClass::UAV :
    ...
    auto validBinding = [&](const DescriptorBinding &binding,
                             const BindingReflection *r) {
      if (!r || binding.descriptor.isEmpty()) return true;
      if (r->d3d12External) {
        Error("RIProgram: external D3D12 descriptors must be written through the bindless set\n");
        return false;
      }
      const RIDescriptor &d = binding.descriptor;
      const auto expected = r->registerClass;
      const bool classOk =
          (d.type == RI_DESCRIPTOR_TYPE_UNIFORM_BUFFER && expected == RIBindlessRegisterClass::CBV) ||
          (d.type == RI_DESCRIPTOR_TYPE_STORAGE_BUFFER &&
           (expected == RIBindlessRegisterClass::SRV || expected == RIBindlessRegisterClass::UAV)) ||
          (d.type == RI_DESCRIPTOR_TYPE_SAMPLED_IMAGE && expected == RIBindlessRegisterClass::SRV) ||
          (d.type == RI_DESCRIPTOR_TYPE_STORAGE_IMAGE && expected == RIBindlessRegisterClass::UAV) ||
          (d.type == RI_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE && expected == RIBindlessRegisterClass::SRV) ||
          (d.type == RI_DESCRIPTOR_TYPE_SAMPLER && expected == RIBindlessRegisterClass::Sampler);
      ...
      if (!classOk || !elementOk) {
        Error("RIProgram: D3D12 descriptor does not match reflected register class/array element\n");
        return false;
      }
      if (d.type == RI_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE)
        return d.payload.accel.gpuVA != 0;
      if (d.type == RI_DESCRIPTOR_TYPE_SAMPLER)
        return d.payload.sampler.initialized != 0 &&
               d.payload.sampler.d3d12Desc.MaxLOD >= d.payload.sampler.d3d12Desc.MinLOD;
      if (d.type == RI_DESCRIPTOR_TYPE_UNIFORM_BUFFER ||
        ...
        if (!resource || !d.payload.buffer.size || !d.payload.buffer.range ||
          ...
          Error("RIProgram: D3D12 buffer descriptor payload is invalid\n");
          return false;
        }
        const bool cbvRoundable = d.payload.buffer.range <= UINT64_MAX - 255u;
        const uint64_t cbvSize = cbvRoundable ?
            ((d.payload.buffer.range + 255u) & ~UINT64(255u)) : 0;
        const ShaderResourceReflection *sr = reflectedResource(r);
        RID3D12BufferShape shape;
        const bool bufferShapeOk = d.type == RI_DESCRIPTOR_TYPE_UNIFORM_BUFFER
        ...
        const uint64_t elementSize = d.type == RI_DESCRIPTOR_TYPE_STORAGE_BUFFER
            ? (shape.raw ? 4 : shape.stride) : 0;
        const uint64_t firstElement = elementSize ? d.payload.buffer.offset / elementSize : 0;
        const uint64_t elementCount = elementSize ? d.payload.buffer.range / elementSize : 0;
        const bool boundOk = d.payload.buffer.offset <= d.payload.buffer.size &&
            d.payload.buffer.range <= d.payload.buffer.size - d.payload.buffer.offset;
        const bool cbvOk = d.type != RI_DESCRIPTOR_TYPE_UNIFORM_BUFFER ||
            (d.payload.buffer.offset % D3D12_CONSTANT_BUFFER_DATA_PLACEMENT_ALIGNMENT == 0 &&
        ...
        const bool elementsOk = d.type == RI_DESCRIPTOR_TYPE_UNIFORM_BUFFER ||
            (elementSize != 0 && firstElement <= UINT_MAX && elementCount != 0 &&
             elementCount <= UINT_MAX);
        if (!boundOk || !cbvOk || !elementsOk || !bufferShapeOk) {
          Error("RIProgram: D3D12 buffer descriptor range/stride is invalid\n");
          return false;
        }
        return true;
      }
      ...
      if (!resource || texture.nativeFormat == DXGI_FORMAT_UNKNOWN ||
        ...
        Error("RIProgram: D3D12 image descriptor payload is invalid\n");
        return false;
      }
      const D3D12_RESOURCE_DESC rd = resource->GetDesc();
      ...
      const bool stateOk = sampled
          ? (texture.state & RI_RESOURCE_STATE_SHADER_RESOURCE) != 0 &&
            (texture.state & (RI_RESOURCE_STATE_STORAGE_READ | RI_RESOURCE_STATE_STORAGE_WRITE |
                              RI_RESOURCE_STATE_COPY_DST | RI_RESOURCE_STATE_COPY_SRC)) == 0
          : (texture.state & (RI_RESOURCE_STATE_GENERAL | RI_RESOURCE_STATE_STORAGE_READ |
                              RI_RESOURCE_STATE_STORAGE_WRITE | RI_RESOURCE_STATE_UNORDERED_ACCESS)) != 0;
      ...
      const bool resourceDimensionOk =
          ((rd.Dimension == D3D12_RESOURCE_DIMENSION_TEXTURE1D) &&
           (viewType == RI_VIEWTYPE_SHADER_RESOURCE_1D || viewType == RI_VIEWTYPE_SHADER_RESOURCE_1D_ARRAY ||
            viewType == RI_VIEWTYPE_SHADER_RESOURCE_STORAGE_1D || viewType == RI_VIEWTYPE_SHADER_RESOURCE_STORAGE_1D_ARRAY)) ||
          ((rd.Dimension == D3D12_RESOURCE_DIMENSION_TEXTURE2D) &&
           (viewType == RI_VIEWTYPE_SHADER_RESOURCE_2D || viewType == RI_VIEWTYPE_SHADER_RESOURCE_2D_ARRAY ||
            viewType == RI_VIEWTYPE_SHADER_RESOURCE_CUBE || viewType == RI_VIEWTYPE_SHADER_RESOURCE_CUBE_ARRAY ||
            viewType == RI_VIEWTYPE_SHADER_RESOURCE_STORAGE_2D || viewType == RI_VIEWTYPE_SHADER_RESOURCE_STORAGE_2D_ARRAY)) ||
          ((rd.Dimension == D3D12_RESOURCE_DIMENSION_TEXTURE3D) &&
           (viewType == RI_VIEWTYPE_SHADER_RESOURCE_3D || viewType == RI_VIEWTYPE_SHADER_RESOURCE_STORAGE_3D));
      const bool is3D = rd.Dimension == D3D12_RESOURCE_DIMENSION_TEXTURE3D;
      const bool layerRangeOk = is3D
      ...
      if (!stateOk || texture.baseMip >= rd.MipLevels ||
           texture.mipNum > rd.MipLevels - texture.baseMip ||
           !layerRangeOk || !viewOk || !resourceDimensionOk) {
        Error("RIProgram: D3D12 image view format/state/range is invalid\n");
        return false;
      }
      const D3D12_RESOURCE_FLAGS required = sampled ? D3D12_RESOURCE_FLAG_NONE
                                                     : D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS;
      if ((sampled && (rd.Flags & D3D12_RESOURCE_FLAG_DENY_SHADER_RESOURCE)) ||
          (required && !(rd.Flags & required))) {
        Error("RIProgram: D3D12 image resource usage is incompatible with descriptor\n");
        return false;
      }
      return true;
    };
    for (size_t i = 0; i < bindingCount; ++i)
      if (!validBinding(bindings[i], lookup(bindings[i]))) return;
    hash_t hash = HASH_INITIAL_VALUE;
    for (const BindingReflection &r : bindingReflection) {
      if (r.d3d12External) continue;
      ...
      for (size_t i = 0; i < bindingCount; ++i) {
        ...
        if (supplied == &r) {
          ...
          hash = hash_u64(hash, bindings[i].descriptor.isEmpty() ? 0 : bindings[i].descriptor.cookie);
        }
      }
    }
    ...
    auto writeEntry = [&](D3D12DescriptorCacheEntry &entry) -> bool {
      ...
      if (!getDescriptorArenaHeaps(device, &rh, &sh)) return false;
      const uint32_t rs = device->d3d12.device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
      const uint32_t ss = device->d3d12.device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER);
      ...
      for (const BindingReflection &r : bindingReflection) {
        ...
        for (uint32_t element = 0; element < count; ++element) {
          if (r.d3d12Sampler) {
            ...
            device->d3d12.device->CreateSampler(&desc, ri_d3d12_cpu(sh, entry.allocation.samplerOffset + r.d3d12DescriptorOffset + element, ss));
          } else {
            D3D12_CPU_DESCRIPTOR_HANDLE cpu = ri_d3d12_cpu(rh, entry.allocation.resourceOffset + r.d3d12DescriptorOffset + element, rs);
            ...
            case RIBindlessRegisterClass::UAV: {
              ...
              const ShaderResourceReflection *sr = reflectedResource(&r);
              const std::string_view type = sr ? std::string_view(sr->type) : std::string_view();
              if (type.find("1D") != std::string_view::npos)
                desc.ViewDimension = r.isArray ? D3D12_UAV_DIMENSION_TEXTURE1DARRAY : D3D12_UAV_DIMENSION_TEXTURE1D;
              else if (type.find("3D") != std::string_view::npos)
                desc.ViewDimension = D3D12_UAV_DIMENSION_TEXTURE3D;
              else if (type.find("2D") != std::string_view::npos)
                desc.ViewDimension = r.isArray ? D3D12_UAV_DIMENSION_TEXTURE2DARRAY : D3D12_UAV_DIMENSION_TEXTURE2D;
              else {
              ...
              device->d3d12.device->CreateUnorderedAccessView(nullptr, nullptr, &desc, cpu); break;
            }
            default: {
              D3D12_SHADER_RESOURCE_VIEW_DESC desc = {};
              const ShaderResourceReflection *sr = reflectedResource(&r);
              const std::string_view type = sr ? std::string_view(sr->type) : std::string_view();
              if (type.find("cube") != std::string_view::npos || type.find("Cube") != std::string_view::npos)
                desc.ViewDimension = r.isArray ? D3D12_SRV_DIMENSION_TEXTURECUBEARRAY : D3D12_SRV_DIMENSION_TEXTURECUBE;
              else if (type.find("1D") != std::string_view::npos)
                desc.ViewDimension = r.isArray ? D3D12_SRV_DIMENSION_TEXTURE1DARRAY : D3D12_SRV_DIMENSION_TEXTURE1D;
              else if (type.find("3D") != std::string_view::npos)
                desc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURE3D;
              else if (type.find("2D") != std::string_view::npos)
                desc.ViewDimension = r.isArray ? D3D12_SRV_DIMENSION_TEXTURE2DARRAY : D3D12_SRV_DIMENSION_TEXTURE2D;
              else {
              ...
              desc.Shader4ComponentMapping = D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING;
              device->d3d12.device->CreateShaderResourceView(nullptr, &desc, cpu); break;
            }
            }
          }
        }
      ...
      for (size_t i = 0; i < bindingCount; ++i) {
        ...
        if (!r || d.isEmpty()) continue;
        if (bindings[i].registerOffset >= std::max(1u, r->descriptorCount) ||
            r->d3d12DescriptorOffset > UINT32_MAX - bindings[i].registerOffset ||
            r->d3d12DescriptorOffset + bindings[i].registerOffset >=
                (r->d3d12Sampler ? entry.allocation.samplerCount : entry.allocation.resourceCount)) {
          Error("RIProgram: D3D12 reflected descriptor offset exceeds allocated table\n");
          return false;
        }
        const uint32_t n = r->d3d12DescriptorOffset + bindings[i].registerOffset;
        if (r->d3d12Sampler) {
          if (d.type != RI_DESCRIPTOR_TYPE_SAMPLER) { Error("RIProgram: D3D12 sampler binding has incompatible descriptor\n"); return false; }
          device->d3d12.device->CreateSampler(&d.payload.sampler.d3d12Desc, ri_d3d12_cpu(sh, entry.allocation.samplerOffset + n, ss));
        } else {
          D3D12_CPU_DESCRIPTOR_HANDLE cpu = ri_d3d12_cpu(rh, entry.allocation.resourceOffset + n, rs);
          ID3D12Resource *resource = d.type == RI_DESCRIPTOR_TYPE_UNIFORM_BUFFER || d.type == RI_DESCRIPTOR_TYPE_STORAGE_BUFFER
              ? d.payload.buffer.nativeResource : d.payload.texture.nativeResource;
          if (d.type == RI_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE) {
            ...
            D3D12_SHADER_RESOURCE_VIEW_DESC v = {};
            if (!ri_d3d12_texture_view(v, d.payload.texture)) { Error("RIProgram: invalid D3D12 sampled-image view payload\n"); return false; }
            ...
            if (!ri_d3d12_texture_uav(v, d.payload.texture, resource)) { Error("RIProgram: invalid D3D12 storage-image view payload\n"); return false; }
            device->d3d12.device->CreateUnorderedAccessView(resource, nullptr, &v, cpu);
          } else if (!resource) { Error("RIProgram: D3D12 descriptor has no native resource payload\n"); return false; }
          else if (d.type == RI_DESCRIPTOR_TYPE_UNIFORM_BUFFER) {
            D3D12_CONSTANT_BUFFER_VIEW_DESC v = { resource->GetGPUVirtualAddress() + d.payload.buffer.offset,
            ...
            RID3D12BufferShape shape;
            if (!ri_d3d12_resolveBufferShape(reflectedResource(r), d.payload.buffer, shape))
              return false;
            const UINT first = (UINT)(d.payload.buffer.offset / (shape.raw ? 4 : shape.stride));
            const UINT elements = (UINT)(d.payload.buffer.range / (shape.raw ? 4 : shape.stride));
            if (r->registerClass == RIBindlessRegisterClass::SRV) {
              D3D12_SHADER_RESOURCE_VIEW_DESC v = {};
              v.ViewDimension = D3D12_SRV_DIMENSION_BUFFER;
              v.Format = shape.raw ? DXGI_FORMAT_R32_TYPELESS : DXGI_FORMAT_UNKNOWN;
              v.Buffer.FirstElement = first; v.Buffer.NumElements = elements;
              v.Buffer.StructureByteStride = shape.raw ? 0 : shape.stride;
              v.Buffer.Flags = shape.raw ? D3D12_BUFFER_SRV_FLAG_RAW : D3D12_BUFFER_SRV_FLAG_NONE;
              v.Shader4ComponentMapping = D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING;
              device->d3d12.device->CreateShaderResourceView(resource, &v, cpu);
            } else {
              D3D12_UNORDERED_ACCESS_VIEW_DESC v = {};
              ...
              v.Buffer.FirstElement = first; v.Buffer.NumElements = elements;
              v.Buffer.StructureByteStride = shape.raw ? 0 : shape.stride;
              v.Buffer.Flags = shape.raw ? D3D12_BUFFER_UAV_FLAG_RAW : D3D12_BUFFER_UAV_FLAG_NONE;
              device->d3d12.device->CreateUnorderedAccessView(resource, nullptr, &v, cpu);
            }
          } else { Error("RIProgram: unsupported D3D12 descriptor payload type\n"); return false; }
        }
      }
      ...
      for (size_t i = 0; i < bindingCount; ++i) {
        ...
        if (!r || d.isEmpty() || r->d3d12Sampler)
          continue;
        const uint32_t slot = r->d3d12DescriptorOffset + bindings[i].registerOffset;
    ...
    if (auto indexed = d3d12DescriptorIndex.find(hash); indexed != d3d12DescriptorIndex.end()) {
      ...
      for (const BindingReflection &r : bindingReflection) {
        ...
        const uint32_t count = std::max(1u, r.descriptorCount);
        if (r.d3d12Sampler) {
          if (samplerCount > UINT32_MAX - count) return;
          samplerCount += count;
        } else {
          if (resourceCount > UINT32_MAX - count) return;
          resourceCount += count;
        }
      }
      for (const BindingReflection &r : bindingReflection) {
        ...
        if (r.d3d12DescriptorOffset > capacity || count > capacity - r.d3d12DescriptorOffset) {
          Error("RIProgram: reflected D3D12 descriptor layout exceeds arena table\n");
          return;
        }
      }
      ...
      const uint32_t cacheSize = static_cast<uint32_t>(d3d12DescriptorCache.size());
      for (uint32_t step = 0; step < cacheSize; ++step) {
        const uint32_t candidate = (d3d12DescriptorRecycleCursor + step) % cacheSize;
        const D3D12DescriptorCacheEntry &entry = d3d12DescriptorCache[candidate];
      ...
      if (entryIndex != UINT32_MAX) {
        ...
        if (!allocateDescriptorArena(device, resourceCount, samplerCount, &entry.allocation)) {
          if (!d3d12ArenaExhaustedReported) {
            d3d12ArenaExhaustedReported = true;
            Error("RIProgram: D3D12 descriptor arena exhausted (%u resource / %u "
      ...
      if (!writeEntry(entry)) {
        ri_d3d12_release_cache_refs(entry.resources, entry.allocations);
        releaseDescriptorArena(device, &entry.allocation, nullptr);
        // Swap-remove, keeping the moved entry's index current.
        const uint32_t last = static_cast<uint32_t>(d3d12DescriptorCache.size() - 1);
        if (entryIndex != last) {
          d3d12DescriptorCache[entryIndex] = std::move(d3d12DescriptorCache[last]);
          d3d12DescriptorIndex[d3d12DescriptorCache[entryIndex].hash] = entryIndex;
        }
    ...
    if (hasResourceTable || hasSamplerTable) {
      if (!getDescriptorArenaHeaps(device, &heaps[0], &heaps[1])) return;
    ...
    if (ri_d3d12ComputeRootPath(bindPoint)) {
      ...
      if (impl.d3d12.resourceRootParameter != UINT32_MAX && hasResourceTable) RID3D12_SetComputeRootDescriptorTable(*cmd, impl.d3d12.resourceRootParameter, ri_d3d12_gpu(heaps[0], allocation.resourceOffset, rs));
      if (impl.d3d12.samplerRootParameter != UINT32_MAX && hasSamplerTable) RID3D12_SetComputeRootDescriptorTable(*cmd, impl.d3d12.samplerRootParameter, ri_d3d12_gpu(heaps[1], allocation.samplerOffset, ss));
    } else {
      cmd->d3d12.computePipelineBound = false;
      RID3D12_SetGraphicsRootSignature(*cmd, impl.d3d12.rootSignature);
      if (impl.d3d12.resourceRootParameter != UINT32_MAX && hasResourceTable) RID3D12_SetGraphicsRootDescriptorTable(*cmd, impl.d3d12.resourceRootParameter, ri_d3d12_gpu(heaps[0], allocation.resourceOffset, rs));
      if (impl.d3d12.samplerRootParameter != UINT32_MAX && hasSamplerTable) RID3D12_SetGraphicsRootDescriptorTable(*cmd, impl.d3d12.samplerRootParameter, ri_d3d12_gpu(heaps[1], allocation.samplerOffset, ss));
    }
    return;
  }
#endif
...
bool RIProgram::bindBindlessDescriptorSet(struct RICmd *cmd,
  ...
  if (RIIsTargetSelected(RI_DEVICE_API_D3D12)) {
    if (!ri_d3d12BindPointSupported(bindPoint))
      return false;
    if (setIndex >= programDescriptors.size() || !programDescriptors[setIndex].isExternal)
      return false;
    ...
    if (!bindless || !cmd || !impl.d3d12.rootSignature ||
        ...
        (programDescriptors[setIndex].d3d12ExternalTables.empty() &&
         ...
         (bindless->d3d12.resourceRootParameter != UINT32_MAX &&
           slot.resourceRootParameter != bindless->d3d12.resourceRootParameter) ||
         ...
         (bindless->d3d12.geometryRootParameter != UINT32_MAX &&
           slot.geometryRootParameter != bindless->d3d12.geometryRootParameter) ||
  ...
  return true;
#endif
  (void)cmd; (void)bindless; (void)setIndex; (void)bindPoint;
  return false;
}

#if (DEVICE_IMPL_D3D12)
bool RIProgram::bindD3D12BindlessDescriptorSet(struct RICmd *cmd,
                                                 RIBindlessDescriptorSet *bindless,
                                                 bool compute,
                                                 uint32_t setIndex) {
  if (!cmd || !cmd->d3d12.cmdList || !bindless || !bindless->d3d12.ownsAllocation)
    return false;
  ...
  if (!externalTables.empty()) {
    ...
    const uint32_t resourceStride = device->d3d12.device->GetDescriptorHandleIncrementSize(
        D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
    const uint32_t samplerStride = device->d3d12.device->GetDescriptorHandleIncrementSize(
    ...
    for (const auto &table : externalTables) {
      ...
      if (binding == bindless->d3d12.bindings.end())
        return false;
      const bool sampler = table.registerClass == RIBindlessRegisterClass::Sampler;
      const uint32_t offset = table.geometryHeap
          ? 0u
          : (sampler ? bindless->d3d12.allocation.samplerOffset
                     : bindless->d3d12.resourceTableBase) + binding->descriptorOffset;
      const D3D12_GPU_DESCRIPTOR_HANDLE handle = sampler
          ? ri_d3d12_gpu(samplerHeap, offset, samplerStride)
          : ri_d3d12_gpu(resourceHeap, offset, resourceStride);
      if (compute)
        RID3D12_SetComputeRootDescriptorTable(*cmd, table.rootParameter, handle);
      else
        RID3D12_SetGraphicsRootDescriptorTable(*cmd, table.rootParameter, handle);
    }
  ...
  const bool hasGeometryTable = impl.d3d12.geometryRootParameter != UINT32_MAX &&
                                bindless->d3d12.geometryRangeCount != 0;
  if (hasResourceTable || hasSamplerTable || hasGeometryTable) {
    if (!getDescriptorArenaHeaps(device, &resourceHeap, &samplerHeap)) return false;
    RID3D12_SetDescriptorHeaps(*cmd, resourceHeap, samplerHeap);
  }
  if (compute) {
        cmd->d3d12.computePipelineBound = true;
    ...
    const uint32_t ss = device->d3d12.device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER);
     if (impl.d3d12.resourceRootParameter != UINT32_MAX && hasResourceTable)
       RID3D12_SetComputeRootDescriptorTable(*cmd, impl.d3d12.resourceRootParameter,
     ...
     if (impl.d3d12.geometryRootParameter != UINT32_MAX &&
       ...
       RID3D12_SetComputeRootDescriptorTable(*cmd, impl.d3d12.geometryRootParameter,
           ri_d3d12_gpu(resourceHeap, 0, rs));
     if (impl.d3d12.samplerRootParameter != UINT32_MAX && hasSamplerTable)
       RID3D12_SetComputeRootDescriptorTable(*cmd, impl.d3d12.samplerRootParameter,
          ri_d3d12_gpu(samplerHeap, bindless->d3d12.allocation.samplerOffset, ss));
  } else {
        cmd->d3d12.computePipelineBound = false;
        RID3D12_SetGraphicsRootSignature(*cmd, impl.d3d12.rootSignature);
    const uint32_t rs = device->d3d12.device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
    const uint32_t ss = device->d3d12.device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER);
     if (impl.d3d12.resourceRootParameter != UINT32_MAX && hasResourceTable)
       RID3D12_SetGraphicsRootDescriptorTable(*cmd, impl.d3d12.resourceRootParameter,
          ri_d3d12_gpu(resourceHeap, bindless->d3d12.resourceTableBase, rs));
     if (impl.d3d12.geometryRootParameter != UINT32_MAX &&
        bindless->d3d12.geometryRangeCount)
       RID3D12_SetGraphicsRootDescriptorTable(*cmd, impl.d3d12.geometryRootParameter,
           ri_d3d12_gpu(resourceHeap, 0, rs));
     if (impl.d3d12.samplerRootParameter != UINT32_MAX && hasSamplerTable)
       RID3D12_SetGraphicsRootDescriptorTable(*cmd, impl.d3d12.samplerRootParameter,
          ri_d3d12_gpu(samplerHeap, bindless->d3d12.allocation.samplerOffset, ss));
  }
  bindless->d3d12.hasBeenBound = true;
  return true;
...
static bool ri_d3d12_registerClass(VkDescriptorType type,
  ...
  case VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC:
    out = RIBindlessRegisterClass::CBV; return true;
  case VK_DESCRIPTOR_TYPE_STORAGE_BUFFER:
  case VK_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC:
    out = RIBindlessRegisterClass::UAV; return true;
  case VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE:
    out = RIBindlessRegisterClass::SRV; return true;
  case VK_DESCRIPTOR_TYPE_STORAGE_IMAGE:
    out = RIBindlessRegisterClass::UAV; return true;
  case VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_KHR:
    out = RIBindlessRegisterClass::SRV; return true;
  case VK_DESCRIPTOR_TYPE_SAMPLER:
    out = RIBindlessRegisterClass::Sampler; return true;
  default:
    return false;
  }
}
...
bool RIBindlessDescriptorSet::convertBinding(const Binding &binding,
  ...
  out.registerSpace = 0;
  out.descriptorType = binding.d3d12DescriptorType == VK_DESCRIPTOR_TYPE_MAX_ENUM
                           ? binding.descriptorType : binding.d3d12DescriptorType;
  ...
  if (out.descriptorType == VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE &&
    ...
    if (out.format == DXGI_FORMAT_UNKNOWN) out.format = DXGI_FORMAT_R8G8B8A8_UNORM;
  }
...
static void ri_d3d12_null_srv(D3D12_SHADER_RESOURCE_VIEW_DESC &desc,
  ...
  case D3D12_SRV_DIMENSION_BUFFER:
    desc.Format = binding.format == DXGI_FORMAT_UNKNOWN ? DXGI_FORMAT_R32_TYPELESS : binding.format;
    desc.Buffer.NumElements = 1;
    desc.Buffer.Flags = desc.Format == DXGI_FORMAT_R32_TYPELESS ? D3D12_BUFFER_SRV_FLAG_RAW : D3D12_BUFFER_SRV_FLAG_NONE;
    break;
  case D3D12_SRV_DIMENSION_TEXTURE1D: desc.Texture1D.MipLevels = 1; break;
  case D3D12_SRV_DIMENSION_TEXTURE1DARRAY: desc.Texture1DArray.MipLevels = 1; desc.Texture1DArray.ArraySize = 1; break;
  case D3D12_SRV_DIMENSION_TEXTURE2D: desc.Texture2D.MipLevels = 1; break;
  ...
  case D3D12_SRV_DIMENSION_TEXTURECUBEARRAY: desc.TextureCubeArray.MipLevels = 1; desc.TextureCubeArray.NumCubes = 1; break;
  case D3D12_SRV_DIMENSION_TEXTURE3D: desc.Texture3D.MipLevels = 1; break;
  case D3D12_SRV_DIMENSION_RAYTRACING_ACCELERATION_STRUCTURE: desc.RaytracingAccelerationStructure.Location = 0; break;
  default: break;
  }
}

static bool ri_d3d12_null_uav(D3D12_UNORDERED_ACCESS_VIEW_DESC &desc,
                              const RIBindlessDescriptorSet::BackendBinding &binding) {
  desc = {};
  desc.Format = binding.format;
  ...
  case D3D12_UAV_DIMENSION_BUFFER:
    desc.Format = binding.format == DXGI_FORMAT_UNKNOWN ? DXGI_FORMAT_R32_TYPELESS : binding.format;
    desc.Buffer.NumElements = 1;
    desc.Buffer.Flags = desc.Format == DXGI_FORMAT_R32_TYPELESS ? D3D12_BUFFER_UAV_FLAG_RAW : D3D12_BUFFER_UAV_FLAG_NONE;
    return true;
  case D3D12_UAV_DIMENSION_TEXTURE1D: return true;
  case D3D12_UAV_DIMENSION_TEXTURE1DARRAY: desc.Texture1DArray.ArraySize = 1; return true;
  case D3D12_UAV_DIMENSION_TEXTURE2D: return true;
  case D3D12_UAV_DIMENSION_TEXTURE2DARRAY: desc.Texture2DArray.ArraySize = 1; return true;
  case D3D12_UAV_DIMENSION_TEXTURE3D: desc.Texture3D.WSize = 1; return true;
...
bool RIBindlessDescriptorSet::initialize(
  ...
  for (const BackendBinding &binding : bindings) {
    ...
    if (!binding.descriptorCount || !seenBindings.insert(binding.binding).second)
      return false;
    if (!geometryBinding &&
    ...
    if (binding.registerClass == RIBindlessRegisterClass::Sampler) {
      ...
      if (binding.descriptorType != VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER &&
          binding.descriptorType != VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC) return false;
      ...
      if (binding.descriptorType != VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE &&
          ...
          binding.descriptorType != VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_KHR) return false;
      if (binding.descriptorType == VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_KHR) {
        if (binding.srvDimension != D3D12_SRV_DIMENSION_RAYTRACING_ACCELERATION_STRUCTURE) return false;
                 ...
                 (binding.srvDimension == D3D12_SRV_DIMENSION_UNKNOWN ||
                  ...
                  binding.format == DXGI_FORMAT_UNKNOWN)) return false;
      if (geometryBinding)
      ...
      if (binding.descriptorType != VK_DESCRIPTOR_TYPE_STORAGE_IMAGE &&
          binding.descriptorType != VK_DESCRIPTOR_TYPE_STORAGE_BUFFER) return false;
      if (binding.descriptorType == VK_DESCRIPTOR_TYPE_STORAGE_IMAGE &&
    ...
    const uint32_t descriptorSpan = geometryBinding ? 0u : binding.descriptorCount;
    switch (binding.registerClass) {
    case RIBindlessRegisterClass::Sampler:
      if (samplers > UINT32_MAX - descriptorSpan) return false;
      samplers += descriptorSpan; break;
    case RIBindlessRegisterClass::CBV:
    case RIBindlessRegisterClass::SRV:
    case RIBindlessRegisterClass::UAV:
      if (resources > UINT32_MAX - descriptorSpan) return false;
      resources += descriptorSpan; break;
    default: return false;
    }
  }
  if (hasGeometryBinding &&
  ...
  if (!allocateDescriptorArena(device, resources, samplers, &d3d12.allocation)) {
    return false;
  }
  ...
  for (BackendBinding &binding : d3d12.bindings) {
    binding.descriptorOffset =
        isGeometryBinding(binding) ? 0u :
        (binding.registerClass == RIBindlessRegisterClass::Sampler
             ? samplerBindingOffset : resourceBindingOffset);
    if (binding.registerClass == RIBindlessRegisterClass::Sampler) {
      const uint32_t descriptorSpan = isGeometryBinding(binding) ? 0u : binding.descriptorCount;
  ...
  for (const BackendBinding &binding : d3d12.bindings) {
    ...
    for (uint32_t n = 0; n < descriptorSpan; ++n) {
      if (binding.registerClass == RIBindlessRegisterClass::Sampler) {
        ...
        desc.Filter = D3D12_FILTER_MIN_MAG_MIP_POINT;
        desc.AddressU = desc.AddressV = desc.AddressW = D3D12_TEXTURE_ADDRESS_MODE_CLAMP;
        device->d3d12.device->CreateSampler(&desc, ri_d3d12_cpu(samplerHeap, samplerOffset++, samplerStride));
      } else {
        D3D12_CPU_DESCRIPTOR_HANDLE cpu = ri_d3d12_cpu(resourceHeap, resourceOffset++, resourceStride);
        switch (binding.registerClass) {
        case RIBindlessRegisterClass::CBV: {
          D3D12_CONSTANT_BUFFER_VIEW_DESC desc = {};
          desc.BufferLocation = 0;
          desc.SizeInBytes = D3D12_CONSTANT_BUFFER_DATA_PLACEMENT_ALIGNMENT;
          device->d3d12.device->CreateConstantBufferView(&desc, cpu); break;
        }
        case RIBindlessRegisterClass::SRV:
        case RIBindlessRegisterClass::UAV:
          if (binding.descriptorType == VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_KHR) {
            ...
            D3D12_UNORDERED_ACCESS_VIEW_DESC desc = {};
            if (!ri_d3d12_null_uav(desc, binding)) { destroy(device); return false; }
            device->d3d12.device->CreateUnorderedAccessView(nullptr, nullptr, &desc, cpu);
          }
          break;
        default: return false;
        }
...
static const RIBindlessDescriptorSet::BackendBinding *ri_find_binding(
    const RIBindlessDescriptorSet &set, uint32_t binding) {
...
void RIBindlessDescriptorSet::destroy(RIDevice *device) {
  ...
  if (d3d12.ownsAllocation) {
    if (d3d12.usesGeometryAllocation)
      ...
      releaseDescriptorArena(device, &d3d12.allocation, nullptr);
...
bool RIBindlessDescriptorSet::writeDescriptors(
  ...
  for (size_t i = 0; i < writes.size(); ++i) {
    ...
    case RI_DESCRIPTOR_TYPE_UNIFORM_BUFFER:
    case RI_DESCRIPTOR_TYPE_STORAGE_BUFFER:
      bufferInfos[i] = {d.vkBuffer(), d.payload.buffer.offset,
                        d.payload.buffer.range};
...
#if (DEVICE_IMPL_D3D12)
  if (RIIsTargetSelected(RI_DEVICE_API_D3D12)) {
  if (!device || !d3d12.ownsAllocation) return false;
  if (writes.empty()) return true;
  ...
  // needs the fence. Applying the gate to the whole set instead rejected every
  // texture loaded after the first frame that bound the set, leaving those
  // slots null while their index still looked valid to the shader.
  // A slot is not recycled until graphicsDefer has drained the index return
  ...
  for (const WriteBinding &write : writes) {
    ...
    bool valid = binding && write.arrayElement < (binding ? binding->descriptorCount : 0);
    if (valid && binding->descriptorOffset > UINT32_MAX - write.arrayElement)
      valid = false;
    if (valid && !write.descriptor.isEmpty()) {
      const enum RIDescriptorType_e type = static_cast<RIDescriptorType_e>(write.descriptor.type);
      ...
      valid = IsValidRIDescriptorType(write.descriptor.type) && descriptorClassOk;
      if (valid && binding->registerClass == RIBindlessRegisterClass::Sampler) {
      ...
      } else if (valid && binding->registerClass == RIBindlessRegisterClass::CBV) {
        const auto &buffer = write.descriptor.payload.buffer;
        const bool roundable = buffer.range <= UINT64_MAX - 255u;
        const uint64_t rounded = roundable ?
            ((buffer.range + 255u) & ~UINT64(255u)) : 0;
        valid = type == RI_DESCRIPTOR_TYPE_UNIFORM_BUFFER &&
                buffer.nativeResource != nullptr && buffer.size != 0 &&
                buffer.range != 0 && rounded != 0 && rounded <= UINT_MAX &&
                buffer.offset % D3D12_CONSTANT_BUFFER_DATA_PLACEMENT_ALIGNMENT == 0 &&
                buffer.offset <= buffer.size &&
                rounded <= buffer.size - buffer.offset;
      } else if (valid && (binding->registerClass == RIBindlessRegisterClass::SRV ||
                           binding->registerClass == RIBindlessRegisterClass::UAV)) {
        if (type == RI_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE) {
          ...
          ID3D12Resource *resource = texture.nativeResource;
          if ((sampled && binding->registerClass != RIBindlessRegisterClass::SRV) ||
              (!sampled && binding->registerClass != RIBindlessRegisterClass::UAV) ||
            ...
            D3D12_SHADER_RESOURCE_VIEW_DESC srv = {};
            D3D12_UNORDERED_ACCESS_VIEW_DESC uav = {};
            const bool viewOk = sampled ? ri_d3d12_texture_view(srv, texture)
                                        : ri_d3d12_texture_uav(uav, texture, resource);
            const bool stateOk = sampled
                ? (texture.state & RI_RESOURCE_STATE_SHADER_RESOURCE) != 0 &&
                  !(texture.state & (RI_RESOURCE_STATE_STORAGE_READ | RI_RESOURCE_STATE_STORAGE_WRITE |
                                     RI_RESOURCE_STATE_COPY_DST | RI_RESOURCE_STATE_COPY_SRC))
                : (texture.state & (RI_RESOURCE_STATE_GENERAL | RI_RESOURCE_STATE_STORAGE_READ |
                                    RI_RESOURCE_STATE_STORAGE_WRITE | RI_RESOURCE_STATE_UNORDERED_ACCESS)) != 0;
            const auto viewDimension = sampled ? srv.ViewDimension : uav.ViewDimension;
            ...
            const bool resourceDimensionOk =
                ...
                ((rd.Dimension == D3D12_RESOURCE_DIMENSION_TEXTURE3D) &&
                 (viewType == RI_VIEWTYPE_SHADER_RESOURCE_3D || viewType == RI_VIEWTYPE_SHADER_RESOURCE_STORAGE_3D));
            const bool layerRangeOk = rd.Dimension == D3D12_RESOURCE_DIMENSION_TEXTURE3D
                ? (texture.baseLayer == 0 && texture.layerNum == 1)
                : (texture.baseLayer < rd.DepthOrArraySize &&
                   texture.layerNum <= rd.DepthOrArraySize - texture.baseLayer);
            ...
            valid = viewOk && stateOk && layerRangeOk && resourceDimensionOk &&
                    viewDimension == (sampled ? binding->srvDimension : binding->uavDimension) &&
                    formatOk && texture.baseMip < rd.MipLevels &&
                    texture.mipNum <= rd.MipLevels - texture.baseMip &&
                    (!sampled || !(rd.Flags & D3D12_RESOURCE_FLAG_DENY_SHADER_RESOURCE)) &&
                    (sampled || (rd.Flags & D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS));
          }
        } else {
          const auto &buffer = write.descriptor.payload.buffer;
          ...
          // shape as raw; explicit stride/structured metadata remains honored.
          const bool raw = buffer.raw ||
                           (!buffer.raw && !buffer.structured && buffer.stride == 0);
          const bool structured = !raw;
          const uint32_t stride = raw ? 0 : buffer.stride;
          const uint64_t elementSize = raw ? 4 : stride;
          const uint64_t firstElement = elementSize ? buffer.offset / elementSize : 0;
          const uint64_t elementCount = elementSize ? buffer.range / elementSize : 0;
          valid = type == RI_DESCRIPTOR_TYPE_STORAGE_BUFFER &&
                  buffer.nativeResource != nullptr && buffer.size != 0 &&
    ...
    if (!valid) {
      hpl::Warning("RIBindlessDescriptorSet::writeDescriptors (D3D12): rejected "
                   "write binding=%u element=%u type=%u%s\n",
                   write.binding, write.arrayElement,
                   unsigned(write.descriptor.type),
                   binding ? "" : " (binding not in set)");
      allWritesValid = false;
      continue;
    }
    // Sampler bindings keep no per-slot resource record, so they stay gated as
    // a whole once the set has been bound. An out-of-range slot is treated as
    // live for the same reason: the write loop below would skip the retain, so
    // there is no evidence the element is untouched.
    if (d3d12.hasBeenBound && !fenceCompleted && !write.descriptor.isEmpty()) {
      const uint64_t slot =
  ...
  if (!allWritesValid) return false;
  ID3D12DescriptorHeap *resourceHeap = nullptr, *samplerHeap = nullptr;
  if (!getDescriptorArenaHeaps(device, &resourceHeap, &samplerHeap)) return false;
  const uint32_t resourceStride = device->d3d12.device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
  const uint32_t samplerStride = device->d3d12.device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER);
  for (const WriteBinding &write : writes) {
    const auto *binding = ri_find_binding(*this, write.binding);
    if (write.descriptor.isEmpty()) continue;
    const uint32_t base = binding->registerClass == RIBindlessRegisterClass::Sampler
        ? d3d12.allocation.samplerOffset : d3d12.resourceTableBase;
    ...
    if (binding->registerClass == RIBindlessRegisterClass::Sampler) {
      if (write.descriptor.type != RI_DESCRIPTOR_TYPE_SAMPLER ||
          ...
          write.descriptor.payload.sampler.d3d12Desc.MaxLOD <
              write.descriptor.payload.sampler.d3d12Desc.MinLOD)
        return false;
      device->d3d12.device->CreateSampler(&write.descriptor.payload.sampler.d3d12Desc,
          ri_d3d12_cpu(samplerHeap, offset, samplerStride));
      continue;
    }
    D3D12_CPU_DESCRIPTOR_HANDLE cpu = ri_d3d12_cpu(resourceHeap, offset, resourceStride);
    ...
    if (binding->registerClass == RIBindlessRegisterClass::CBV &&
      ...
      desc.BufferLocation = descriptor.payload.buffer.nativeResource
          ? descriptor.payload.buffer.nativeResource->GetGPUVirtualAddress() + descriptor.payload.buffer.offset : 0;
      desc.SizeInBytes = (UINT)((descriptor.payload.buffer.range + 255) & ~UINT64(255));
      device->d3d12.device->CreateConstantBufferView(&desc, cpu);
      ...
      desc.ViewDimension = D3D12_SRV_DIMENSION_RAYTRACING_ACCELERATION_STRUCTURE;
      desc.Shader4ComponentMapping = D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING;
      desc.RaytracingAccelerationStructure.Location = descriptor.payload.accel.gpuVA;
      device->d3d12.device->CreateShaderResourceView(nullptr, &desc, cpu);
    } else if (descriptor.type == RI_DESCRIPTOR_TYPE_SAMPLED_IMAGE &&
               binding->registerClass == RIBindlessRegisterClass::SRV) {
      D3D12_SHADER_RESOURCE_VIEW_DESC desc = {};
      if (!ri_d3d12_texture_view(desc, descriptor.payload.texture)) return false;
      device->d3d12.device->CreateShaderResourceView(descriptor.payload.texture.nativeResource, &desc, cpu);
    } else if (descriptor.type == RI_DESCRIPTOR_TYPE_STORAGE_IMAGE &&
               binding->registerClass == RIBindlessRegisterClass::UAV) {
      D3D12_UNORDERED_ACCESS_VIEW_DESC desc = {};
      if (!ri_d3d12_texture_uav(desc, descriptor.payload.texture, descriptor.payload.texture.nativeResource)) return false;
      device->d3d12.device->CreateUnorderedAccessView(descriptor.payload.texture.nativeResource, nullptr, &desc, cpu);
    } else if ((binding->registerClass == RIBindlessRegisterClass::SRV ||
                binding->registerClass == RIBindlessRegisterClass::UAV) &&
      ...
      const uint64_t elements = descriptor.payload.buffer.range / (raw ? 4 : stride);
      if (binding->registerClass == RIBindlessRegisterClass::SRV) {
        D3D12_SHADER_RESOURCE_VIEW_DESC desc = {};
        desc.ViewDimension = D3D12_SRV_DIMENSION_BUFFER;
        desc.Format = raw ? DXGI_FORMAT_R32_TYPELESS : DXGI_FORMAT_UNKNOWN;
        desc.Buffer.FirstElement = descriptor.payload.buffer.offset / (raw ? 4 : stride);
        ...
        desc.Shader4ComponentMapping = D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING;
        device->d3d12.device->CreateShaderResourceView(descriptor.payload.buffer.nativeResource, &desc, cpu);
      } else {
        D3D12_UNORDERED_ACCESS_VIEW_DESC desc = {};
        desc.ViewDimension = D3D12_UAV_DIMENSION_BUFFER;
        desc.Format = raw ? DXGI_FORMAT_R32_TYPELESS : DXGI_FORMAT_UNKNOWN;
        ...
        device->d3d12.device->CreateUnorderedAccessView(descriptor.payload.buffer.nativeResource, nullptr, &desc, cpu);
      }
    }

    const uint32_t resourceSlot = binding->descriptorOffset + write.arrayElement;
...
#endif
  (void)device; (void)writes; (void)completionFence;
  return false;
}
...
RIProgram::ShaderArtifact RIProgram::loadShaderArtifact(cFileSearcher *searcher, const tString& asName,
  ...
  artifact.format = ri_detectShaderFormat(std::span<const char>(bytes->data(), bytes->size()));
  ...
  const RIShaderArtifactFormat format =
      ri_readShaderMetadata(searcher, artifactName, result.size(), &reflectionJson,
                            &artifactMeta);
  metadataFormat = format;
  const RIShaderArtifactFormat expected = RIShaderArtifactFormat::Dxil;
  if (format != expected)
    FatalError("RIProgram: shader metadata format does not match active backend for '%s'\n",
               artifactName.c_str());
  if (RIIsTargetSelected(RI_DEVICE_API_D3D12) &&
      ri_detectShaderFormat(std::span<const char>(bytes->data(), bytes->size())) != format)
    FatalError("RIProgram: shader artifact bytes do not match metadata for '%s'\n",
               artifactName.c_str());
...
RIProgram::ShaderArtifact RIProgram::loadShaderStage(cFileSearcher *searcher, const tString &asName,
                                                     const char *entryPoint) {
...
static bool ri_create_reflected_d3d12_root_signature(
    RIDevice *device, const std::array<RIProgram::ShaderBinary, RIProgram::PROGRAM_STAGES_MAX> &bins,
    std::span<const RIBindlessLayout> externalLayouts,
    std::array<RIProgram::DescriptorSetSlot, RIProgram::DESCRIPTOR_SET_MAX> *descriptorSlots,
  ...
  struct Key {
    RIProgram::ShaderRegisterClass cls; uint32_t reg, space, count; std::string name;
  ...
  auto reflectedClass = [](RIProgram::ShaderRegisterClass cls) {
    switch (cls) {
    case RIProgram::ShaderRegisterClass::CBV: return RIBindlessRegisterClass::CBV;
    case RIProgram::ShaderRegisterClass::SRV: return RIBindlessRegisterClass::SRV;
    case RIProgram::ShaderRegisterClass::UAV: return RIBindlessRegisterClass::UAV;
    case RIProgram::ShaderRegisterClass::Sampler: return RIBindlessRegisterClass::Sampler;
    default: return RIBindlessRegisterClass::SRV;
    }
  };
  auto findExternal = [&](const RIProgram::ShaderResourceReflection &resource,
    ...
    bindingOut = nullptr;
    for (uint32_t set = 0; set < externalLayouts.size() &&
                           set < RIProgram::DESCRIPTOR_SET_MAX; ++set) {
      for (const auto &binding : externalLayouts[set].d3d12.bindings) {
        if (binding.registerClass != reflectedClass(resource.registerClass) ||
          ...
          continue;
        const uint32_t needed = resource.unbounded ? 1u : std::max(1u, resource.arrayCount);
        if (!binding.descriptorCount ||
            (binding.descriptorCount != UINT_MAX && binding.descriptorCount < needed))
          FatalError("RIProgram: external D3D12 binding '%s' has insufficient descriptors\n",
                     resource.name.c_str());
        if (bindingOut)
          FatalError("RIProgram: reflected D3D12 resource '%s' matches multiple external layouts\n",
                     resource.name.c_str());
        setOut = set;
        bindingOut = &binding;
      }
  ...
  if (bin.reflection && !bin.buf.empty()) programStages |= stageBit;
  if (bin.reflection) for (const auto &r : bin.reflection->resources) {
    if (!r.used || r.name.empty()) continue;
    ...
    if (mergedIt == merged.end()) {
      ...
      const bool shapeConflict =
          ...
          prior.arrayCount != r.arrayCount || prior.unbounded != r.unbounded ||
          (prior.stride && r.stride && prior.stride != r.stride) ||
          (prior.size && r.size && prior.size != r.size) ||
          (!prior.type.empty() && !r.type.empty() && prior.type != r.type) ||
          (!prior.format.empty() && !r.format.empty() && prior.format != r.format);
      if (shapeConflict)
        FatalError("RIProgram: conflicting reflected D3D12 resource '%s' across shader stages\n",
                   r.name.c_str());
    }
    RIProgram::ShaderRegisterClass cls = r.registerClass;
    if (cls == RIProgram::ShaderRegisterClass::Unknown)
      FatalError("RIProgram: reflected D3D12 resource has no register class\n");
    const uint32_t count = r.unbounded ? UINT_MAX : std::max(1u, r.arrayCount);
    // D3D12 keeps CBV, SRV, UAV, and sampler registers in independent
    // namespaces. b0, t0, u0, and s0 may therefore coexist in the same space.
    auto it = std::find_if(keys.begin(), keys.end(), [&](const Key &k) { return k.cls == cls && k.reg == r.registerIndex && k.space == r.registerSpace; });
    if (it != keys.end()) {
      if (it->name != r.name)
        FatalError("RIProgram: conflicting reflected D3D12 resources '%s' and '%s' share register %u/space%u\n",
                   it->name.c_str(), r.name.c_str(), r.registerIndex, r.registerSpace);
      if (it->count != count) FatalError("RIProgram: conflicting reflected D3D12 descriptor layouts\n");
    ...
    key.stages = stageBit;
    D3D12_DESCRIPTOR_RANGE1 range{};
    range.RangeType = cls == RIProgram::ShaderRegisterClass::CBV ? D3D12_DESCRIPTOR_RANGE_TYPE_CBV : cls == RIProgram::ShaderRegisterClass::SRV ? D3D12_DESCRIPTOR_RANGE_TYPE_SRV : cls == RIProgram::ShaderRegisterClass::UAV ? D3D12_DESCRIPTOR_RANGE_TYPE_UAV : D3D12_DESCRIPTOR_RANGE_TYPE_SAMPLER;
    range.NumDescriptors = count; range.BaseShaderRegister = r.registerIndex; range.RegisterSpace = r.registerSpace; range.OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;
    ...
    if (findExternal(r, externalSet, externalBinding)) {
      ...
      external.push_back(
          {range, externalSet, externalBinding->binding,
           externalBinding->registerClass, externalBinding->geometryHeap});
    } else {
  ...
  for (const Key &key : keys) {
    switch (key.table) {
    case KeyTable::Resource: resourceStages |= key.stages; break;
    case KeyTable::Sampler: samplerStages |= key.stages; break;
    case KeyTable::External: external[key.externalIndex].stages |= key.stages; break;
    }
  }
  ...
  const bool graphicsProgram = (programStages & (kVertexBit | kFragmentBit)) != 0 &&
                               (programStages & ~(kVertexBit | kFragmentBit)) == 0;
  auto visibilityFor = [&](uint32_t stages) {
    if (!graphicsProgram) return D3D12_SHADER_VISIBILITY_ALL;
    if (stages == kVertexBit) return D3D12_SHADER_VISIBILITY_VERTEX;
    if (stages == kFragmentBit) return D3D12_SHADER_VISIBILITY_PIXEL;
    return D3D12_SHADER_VISIBILITY_ALL;
  };
  auto enforceUnboundedRangeOrder = [](auto &ranges, const char *tableName) {
    const size_t unboundedCount = static_cast<size_t>(std::count_if(
        ranges.begin(), ranges.end(), [](const auto &range) {
          return range.NumDescriptors == UINT_MAX;
        }));
    if (unboundedCount > 1)
      FatalError("RIProgram: D3D12 %s table has multiple unbounded descriptor ranges\n",
                 tableName);
  ...
  *geometryRangeCount = 0;
  ...
  uint32_t reflectedPushOffset = 0, reflectedPushSize = 0, reflectedPushRegister = 0,
           reflectedPushSpace = 0;
  bool reflectedPush = false;
  for (size_t stageIndex = 0; stageIndex < bins.size(); ++stageIndex) {
    const auto &bin = bins[stageIndex];
    if (!bin.reflection || !bin.reflection->pushConstants.present) continue;
    pushStages |= 1u << stageIndex;
    const auto &push = bin.reflection->pushConstants;
    if (!push.size || (push.offset & 3u) || (push.size & 3u) ||
        push.offset > UINT32_MAX - push.size ||
        (push.offset + push.size) / 4 > 64)
      FatalError("RIProgram: D3D12 push-constant reflection is invalid or exceeds 64 DWORDs\n");
    if (!reflectedPush) {
      reflectedPushOffset = push.offset; reflectedPushSize = push.size;
      reflectedPushRegister = push.registerIndex; reflectedPushSpace = push.registerSpace;
      reflectedPush = true;
    } else if (reflectedPushOffset != push.offset || reflectedPushSize != push.size ||
               reflectedPushRegister != push.registerIndex || reflectedPushSpace != push.registerSpace) {
      FatalError("RIProgram: conflicting reflected D3D12 push-constant ranges\n");
    }
  }
  if (!resources.empty()) {
  ...
  *pushConstantParameter = UINT32_MAX;
  ...
  if (graphicsProgram) {
    ...
    if (!(programStages & kVertexBit))
      desc.Desc_1_1.Flags |= D3D12_ROOT_SIGNATURE_FLAG_DENY_VERTEX_SHADER_ROOT_ACCESS;
    if (!(programStages & kFragmentBit))
      desc.Desc_1_1.Flags |= D3D12_ROOT_SIGNATURE_FLAG_DENY_PIXEL_SHADER_ROOT_ACCESS;
  }
  ID3DBlob *serialized=nullptr,*error=nullptr;
  const bool ok=D3D12_WrapResult(D3D12SerializeVersionedRootSignature(&desc,&serialized,&error)) && serialized && D3D12_WrapResult(device->d3d12.device->CreateRootSignature(0,serialized->GetBufferPointer(),serialized->GetBufferSize(),IID_PPV_ARGS(out)));
  if (!ok) {
    // The serializer explains a rejected layout only through this blob.
    hpl::Error("RIProgram: D3D12 root signature rejected (%u params, push constants %u DWORDs @ b%u space%u): %s\n",
               unsigned(params.size()),
               reflectedPush ? unsigned((reflectedPushOffset + reflectedPushSize) / 4) : 0u,
               reflectedPushRegister, reflectedPushSpace,
               error ? static_cast<const char *>(error->GetBufferPointer()) : "<no serializer message>");
  }
  if (error) error->Release(); if (serialized) serialized->Release(); return ok;
}
#endif
...
void RIProgram::initialize(RIDevice* device, std::span<ModuleStage> moduleInit,
  ...
  if (RIIsTargetSelected(RI_DEVICE_API_D3D12)) {
    ...
    for (auto &init : moduleInit) {
      ...
      if (init.format == RIShaderArtifactFormat::Unknown)
        init.format = ri_detectShaderFormat(std::span<const char>(init.data.data(), init.data.size()));
      ...
      if (!init.artifact.reflection)
        FatalError("RIProgram: D3D12 shader stage has no retained reflection metadata\n");
      ...
      if (bin->reflection && init.entryPoint && bin->reflection->json)
        bin->reflection = ri_parseShaderReflection(*bin->reflection->json,
                                                    init.entryPoint,
                                                    ri_stageName(init.stage));
      if (!bin->reflection)
        FatalError("RIProgram: DXIL reflection stage is incompatible with requested shader stage\n");
    ...
    if (rayTracingProgram ||
        !D3D12_WrapResult(D3DGetBlobPart(
            rootStage->buf.data(), rootStage->buf.size(), D3D_BLOB_ROOT_SIGNATURE,
            0, &rootBlob)) || !rootBlob) {
      if (!ri_create_reflected_d3d12_root_signature(
              device, shaderBin, externalLayouts, &programDescriptors,
              &impl.d3d12.rootSignature, &impl.d3d12.resourceRootParameter,
              &impl.d3d12.geometryRootParameter,
              &impl.d3d12.samplerRootParameter,
              &impl.d3d12.geometryRangeOffset,
              &impl.d3d12.geometryRangeCount,
              &impl.d3d12.pushConstantRootParameter,
              &impl.d3d12.pushConstantOffset,
              &impl.d3d12.pushConstantSize))
        FatalError("RIProgram '%s': DXIL has no embedded root signature and reflection root creation failed\n",
                   debugName ? debugName : "<unnamed program>");
    }
    ID3D12VersionedRootSignatureDeserializer *deserializer = nullptr;
    if (rootBlob && !D3D12_WrapResult(D3D12CreateVersionedRootSignatureDeserializer(
    ...
    const D3D12_VERSIONED_ROOT_SIGNATURE_DESC *rootDesc = deserializer ?
        deserializer->GetUnconvertedRootSignatureDesc() : nullptr;
    auto validTableParameter = [&](uint32_t parameter) {
      if (parameter == UINT32_MAX) return true;
      ...
      if (!rootDesc) return true;
      switch (rootDesc->Version) {
      case D3D_ROOT_SIGNATURE_VERSION_1_0:
        return parameter < rootDesc->Desc_1_0.NumParameters && rootDesc->Desc_1_0.pParameters[parameter].ParameterType == D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE && rootDesc->Desc_1_0.pParameters[parameter].DescriptorTable.NumDescriptorRanges > 0;
      case D3D_ROOT_SIGNATURE_VERSION_1_1:
        return parameter < rootDesc->Desc_1_1.NumParameters && rootDesc->Desc_1_1.pParameters[parameter].ParameterType == D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE && rootDesc->Desc_1_1.pParameters[parameter].DescriptorTable.NumDescriptorRanges > 0;
      default:
        return parameter < rootDesc->Desc_1_2.NumParameters && rootDesc->Desc_1_2.pParameters[parameter].ParameterType == D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE && rootDesc->Desc_1_2.pParameters[parameter].DescriptorTable.NumDescriptorRanges > 0;
      }
    };
    bool shaderHasGeometry = false;
    bool shaderHasResources = false;
    bool shaderHasSamplers = false;
    for (const auto &bin : shaderBin) if (bin.reflection) for (const auto &r : bin.reflection->resources)
      if (r.used && !r.name.empty()) {
        ...
        shaderHasSamplers |= r.registerClass == ShaderRegisterClass::Sampler;
        shaderHasResources |= r.registerClass != ShaderRegisterClass::Sampler && !geometry;
      }
    ID3DBlob *serializedRoot = nullptr;
    ID3DBlob *rootError = nullptr;
    bool serialized = !deserializer;
    if (deserializer) {
      serialized = rootDesc && D3D12_WrapResult(D3D12SerializeVersionedRootSignature(
    ...
    if (!rootCreated) {
      ...
      if (rootError) rootError->Release();
      if (serializedRoot) serializedRoot->Release();
      if (deserializer) deserializer->Release();
      rootBlob->Release();
    ...
    bool reflectedPush = false;
    uint32_t reflectedPushOffset = 0, reflectedPushSize = 0,
             reflectedPushRegister = 0, reflectedPushSpace = 0;
    for (const auto &bin : shaderBin) if (bin.reflection && bin.reflection->pushConstants.present) {
      const auto &push = bin.reflection->pushConstants;
      if (!push.size || (push.offset & 3u) || (push.size & 3u) ||
          push.offset > UINT32_MAX - push.size ||
          (push.offset + push.size) / 4 > 64)
        FatalError("RIProgram: D3D12 push-constant reflection is invalid or exceeds 64 DWORDs\n");
      if (!reflectedPush) {
        reflectedPush = true; reflectedPushOffset = push.offset; reflectedPushSize = push.size;
        reflectedPushRegister = push.registerIndex; reflectedPushSpace = push.registerSpace;
      } else if (reflectedPushOffset != push.offset || reflectedPushSize != push.size ||
                 reflectedPushRegister != push.registerIndex || reflectedPushSpace != push.registerSpace) {
        FatalError("RIProgram: conflicting reflected D3D12 push-constant ranges\n");
    ...
    if (deserializer && reflectedPush) {
      ...
      auto matchesPush = [&](UINT parameterCount, const auto *params) {
        for (UINT p = 0; p < parameterCount; ++p) {
          ...
          if (param.ParameterType != D3D12_ROOT_PARAMETER_TYPE_32BIT_CONSTANTS) continue;
      ...
      switch (rootDesc->Version) {
      case D3D_ROOT_SIGNATURE_VERSION_1_0:
        matched = matchesPush(rootDesc->Desc_1_0.NumParameters, rootDesc->Desc_1_0.pParameters); break;
      case D3D_ROOT_SIGNATURE_VERSION_1_1:
        matched = matchesPush(rootDesc->Desc_1_1.NumParameters, rootDesc->Desc_1_1.pParameters); break;
      default:
        matched = matchesPush(rootDesc->Desc_1_2.NumParameters, rootDesc->Desc_1_2.pParameters); break;
      }
      if (!matched)
        FatalError("RIProgram: embedded D3D12 root signature has no reflected push-constant parameter\n");
    }
    ...
    if (deserializer) {
      ...
      auto tableCovers = [&](uint32_t parameter, bool sampler, bool geometryOnly = false) {
        auto rangeCovers = [&](D3D12_DESCRIPTOR_RANGE_TYPE actualType,
                               D3D12_DESCRIPTOR_RANGE_TYPE expectedType, UINT base,
                               UINT count, UINT space, UINT regSpace, UINT reg, UINT needed) {
          return actualType == expectedType &&
                 space == regSpace && base <= reg && (count == UINT_MAX || reg - base + needed <= count);
        };
        bool ok = true, saw = false;
        auto inspect = [&](auto *params, UINT count, auto versionTag) {
          (void)versionTag;
          if (parameter >= count || params[parameter].ParameterType != D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE) return;
          const auto &table = params[parameter].DescriptorTable;
          for (const auto &bin : shaderBin) if (bin.reflection) for (const auto &r : bin.reflection->resources) {
             const bool isGeometry = r.registerClass == ShaderRegisterClass::SRV &&
                 r.registerIndex == 4 && r.registerSpace == 3 && r.unbounded;
             if (!r.used || r.name.empty() || (r.registerClass == ShaderRegisterClass::Sampler) != sampler ||
                 isGeometry != geometryOnly) continue;
            const auto type = r.registerClass == ShaderRegisterClass::CBV ? D3D12_DESCRIPTOR_RANGE_TYPE_CBV : r.registerClass == ShaderRegisterClass::SRV ? D3D12_DESCRIPTOR_RANGE_TYPE_SRV : r.registerClass == ShaderRegisterClass::UAV ? D3D12_DESCRIPTOR_RANGE_TYPE_UAV : D3D12_DESCRIPTOR_RANGE_TYPE_SAMPLER;
            const UINT needed = r.unbounded ? UINT_MAX : std::max(1u, r.arrayCount);
            bool found = false;
            for (UINT i = 0; i < table.NumDescriptorRanges; ++i) {
              const auto &range = table.pDescriptorRanges[i];
              if (rangeCovers(range.RangeType, type, range.BaseShaderRegister,
        ...
        switch (rootDesc->Version) {
        case D3D_ROOT_SIGNATURE_VERSION_1_0: inspect(rootDesc->Desc_1_0.pParameters, rootDesc->Desc_1_0.NumParameters, 0); break;
        case D3D_ROOT_SIGNATURE_VERSION_1_1: inspect(rootDesc->Desc_1_1.pParameters, rootDesc->Desc_1_1.NumParameters, 0); break;
        default: inspect(rootDesc->Desc_1_2.pParameters, rootDesc->Desc_1_2.NumParameters, 0); break;
        }
        return saw && ok;
      };
      ...
      impl.d3d12.geometryRangeOffset = UINT32_MAX;
      impl.d3d12.geometryRangeCount = 0;
      UINT parameterCount = rootDesc->Version == D3D_ROOT_SIGNATURE_VERSION_1_0 ? rootDesc->Desc_1_0.NumParameters : rootDesc->Version == D3D_ROOT_SIGNATURE_VERSION_1_1 ? rootDesc->Desc_1_1.NumParameters : rootDesc->Desc_1_2.NumParameters;
      for (UINT p = 0; p < parameterCount; ++p) {
         if (tableCovers(p, false)) impl.d3d12.resourceRootParameter = p;
         if (tableCovers(p, false, true)) impl.d3d12.geometryRootParameter = p;
         if (tableCovers(p, true)) impl.d3d12.samplerRootParameter = p;
      }
      if ((shaderHasResources && !validTableParameter(impl.d3d12.resourceRootParameter)) ||
          (shaderHasGeometry && !validTableParameter(impl.d3d12.geometryRootParameter)) ||
          (shaderHasSamplers && !validTableParameter(impl.d3d12.samplerRootParameter)))
        FatalError("RIProgram: embedded D3D12 root signature conflicts with reflected bindings\n");
      ...
      if (shaderHasGeometry && !geometryCaptured)
        FatalError("RIProgram: embedded D3D12 root signature has no t4/space3 geometry range\n");
      if (shaderHasGeometry &&
          (impl.d3d12.geometryRootParameter == impl.d3d12.resourceRootParameter ||
           impl.d3d12.geometryRootParameter == impl.d3d12.samplerRootParameter))
        FatalError("RIProgram: embedded D3D12 geometry range is not in a dedicated root table\n");
    }
    for (size_t set = 0; set < externalLayouts.size() &&
                         set < programDescriptors.size(); ++set) {
      ...
      if (!layout.bindings.empty()) {
        if (layout.rootSignature &&
          ...
          FatalError("RIProgram: external D3D12 layout was built for a different root signature\n");
      ...
      // sufficient because descriptor ranges are part of the root signature.
       if ((layout.rootSignature && layout.rootSignature != impl.d3d12.rootSignature) ||
            (layout.rootSignature && shaderHasResources && !validTableParameter(layout.resourceRootParameter)) ||
            (layout.rootSignature && shaderHasGeometry && !validTableParameter(layout.geometryRootParameter)) ||
            (layout.rootSignature && shaderHasSamplers && !validTableParameter(layout.samplerRootParameter)) ||
            (layout.rootSignature && layout.resourceRootParameter != impl.d3d12.resourceRootParameter) ||
            (layout.rootSignature && layout.geometryRootParameter != impl.d3d12.geometryRootParameter) ||
            (layout.rootSignature && layout.samplerRootParameter != impl.d3d12.samplerRootParameter) ||
        ...
        FatalError("RIProgram: external D3D12 layout is incompatible with the program root signature\n");
      programDescriptors[set].isExternal = true;
       programDescriptors[set].d3d12.rootSignature = impl.d3d12.rootSignature;
       programDescriptors[set].d3d12.resourceRootParameter = impl.d3d12.resourceRootParameter;
       programDescriptors[set].d3d12.geometryRootParameter = impl.d3d12.geometryRootParameter;
      programDescriptors[set].d3d12.samplerRootParameter = impl.d3d12.samplerRootParameter;
      programDescriptors[set].d3d12.geometryRangeOffset = layout.geometryRangeOffset;
    }
    if (deserializer) deserializer->Release();
    ...
    // never trips the draw-time check. Parameters past 63 are not tracked.
    {
      ...
      require(impl.d3d12.pushConstantRootParameter, impl.d3d12.pushConstantSize != 0);
    ...
    auto externalBindingFor = [&](const ShaderResourceReflection &resource,
                                  uint32_t &setOut)
        -> const RIBindlessD3D12Binding * {
      ...
      for (uint32_t set = 0; set < externalLayouts.size() &&
        ...
        for (const auto &binding : externalLayouts[set].d3d12.bindings) {
          if (binding.registerClass != registerClass ||
            ...
            continue;
          if (match)
            FatalError("RIProgram: reflected D3D12 binding '%s' matches multiple external layouts\n",
                       resource.name.c_str());
          match = &binding;
          setOut = set;
        }
    ...
    for (const auto &bin : shaderBin) {
      if (!bin.reflection) continue;
      const auto &reflection = *bin.reflection;
      if (reflection.pushConstants.present) {
        hasPushConstant = true;
        reflection_len = static_cast<uint16_t>(std::min<uint32_t>(reflection.pushConstants.size, UINT16_MAX));
      }
      for (const auto &resource : reflection.resources) {
        if (!resource.used || resource.name.empty()) continue;
        ...
        if (mergedIt == mergedResources.end()) {
          ...
          const bool shapeConflict =
              ...
              (prior.stride && resource.stride && prior.stride != resource.stride) ||
              (prior.size && resource.size && prior.size != resource.size) ||
              (!prior.type.empty() && !resource.type.empty() && prior.type != resource.type) ||
              (!prior.format.empty() && !resource.format.empty() && prior.format != resource.format);
          if (shapeConflict)
            FatalError("RIProgram: conflicting reflected D3D12 resource '%s' across shader stages\n",
                       resource.name.c_str());
        }
        const DescriptorBindingID id = CreateDescriptorBindingID(resource.name.c_str());
        auto it = std::find_if(bindingReflection.begin(), bindingReflection.end(), [&](const BindingReflection &r) { return r.hash == id.hash; });
        const bool newReflection = it == bindingReflection.end();
        if (newReflection) { bindingReflection.push_back({}); it = std::prev(bindingReflection.end()); }
        uint32_t externalSet = UINT32_MAX;
        const RIBindlessD3D12Binding *externalBinding =
            externalBindingFor(resource, externalSet);
        const uint32_t logicalSet = externalBinding
            ? externalSet
            : std::min<uint32_t>(resource.registerSpace, 7u);
        const uint32_t logicalBinding = externalBinding
        ...
        it->dimCount = static_cast<uint16_t>(std::min<uint32_t>(resource.unbounded ? 0 : resource.arrayCount, 255));
        switch (resource.registerClass) {
        case ShaderRegisterClass::CBV: it->registerClass = RIBindlessRegisterClass::CBV; break;
        case ShaderRegisterClass::SRV: it->registerClass = RIBindlessRegisterClass::SRV; break;
        case ShaderRegisterClass::UAV: it->registerClass = RIBindlessRegisterClass::UAV; break;
        case ShaderRegisterClass::Sampler: it->registerClass = RIBindlessRegisterClass::Sampler; break;
        default: FatalError("RIProgram: conflicting D3D12 reflection register class\n");
        }
        it->d3d12Sampler = it->registerClass == RIBindlessRegisterClass::Sampler;
        it->descriptorCount = std::max(1u, resource.arrayCount);
        it->d3d12External = externalBinding != nullptr;
        it->d3d12ExternalSet = externalBinding
            ? static_cast<uint8_t>(externalSet)
            : UINT8_MAX;
        if (!newReflection &&
            (it->registerClass != (resource.registerClass == ShaderRegisterClass::CBV ? RIBindlessRegisterClass::CBV :
                                         resource.registerClass == ShaderRegisterClass::SRV ? RIBindlessRegisterClass::SRV :
                                         resource.registerClass == ShaderRegisterClass::UAV ? RIBindlessRegisterClass::UAV :
                                         RIBindlessRegisterClass::Sampler) ||
                   it->baseRegisterIndex != logicalBinding ||
                   it->set != logicalSet ||
                   it->descriptorCount != std::max(1u, resource.arrayCount) ||
                   it->isArray != (resource.unbounded || resource.arrayCount > 1))) {
          FatalError("RIProgram: conflicting reflected D3D12 binding '%s' across shader stages\n",
                     resource.name.c_str());
        }
#if !defined(NDEBUG)
    ...
    // The reflected root-table builder keeps bounded ordinary ranges first and the
    // single unbounded range last. Rebuild compact-arena offsets from that
      ...
      for (int unboundedPass = 0; unboundedPass != 2; ++unboundedPass) {
        for (BindingReflection &binding : bindingReflection) {
          ...
          if (offset > UINT32_MAX - binding.descriptorCount)
            FatalError("RIProgram: reflected D3D12 descriptor table is too large\n");
  ...
  for (auto &init : moduleInit) {
    ...
    if (format != RIShaderArtifactFormat::Spirv)
      FatalError("RIProgram: Vulkan program requires SPIR-V shader artifacts\n");
    auto *bin = &shaderBin[init.stage];
...
void RIProgram::dispose(RIDevice *device) {
  ...
  if (RIIsTargetSelected(RI_DEVICE_API_D3D12)) {
    ...
    for (auto &[hash, slot] : pipeline) {
      if (slot.d3d12.handle) { slot.d3d12.handle->Release(); slot.d3d12.handle = nullptr; }
