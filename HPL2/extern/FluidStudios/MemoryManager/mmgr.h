...
#if MMGR_BACKTRACE && !defined(MMGR_BACKTRACE_SIZE)
...
extern "C"
{
	...
	/* Legacy owner hints are thread-local and latest-call-wins. Explicit
	   allocation arguments always supply the recorded owner. A successful
	   allocation/reallocation or completed deallocation clears the hints. */
	void mmgrSetOwner(const char* file, const unsigned int line, const char* func);

	// ---------------------------------------------------------------------------------------------------------------------------------
	// Locked options.  These setters copy the requested value while the
	// allocation lock is held; no pointer to manager-owned state escapes.
	// ---------------------------------------------------------------------------------------------------------------------------------
	...
	/* Returns true for any byte in a live reported range.  Unlike
	   mmgrValidateAddress, this is an interior-range query and never grants
	   ownership to an exact-base deallocation. */
	...
	/* These functions inspect caller-owned payload bytes.  The caller must
	   synchronize with all application writers; ordinary reports do not use
	   them. */
